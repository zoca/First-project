...
class PostsController extends Controller
{
    ...
    public function datatable()
    {
        return
            ...
            ->editColumn('photo', function ($entity) {
                return view($this->namespace . 'partials.table.photo', compact('entity'));
            })
            ...
            ->orderColumn('order_number', 'order_number $1') 
            ->make(true);
    ...
    public function update(Entity $entity)
    {
        ...
        $data = $request->validate([
            ...
            'tags' => 'nullable|array|min:1',
            'tags.*' => 'nullable|integer',
            'comments' => 'nullable|array|min:1',
            'comments.*' => 'nullable|string|min:10|max:655',
            'status' => [
                'nullable', 'string', 'in:' . implode(',', Entity::STATUSES),
                // function ($attribute, $value, $fail) {
                //     if ($value === 'foo') {
        ...
        if (request()->has('comments')) {
            foreach ($data['comments'] as $commentId => $commentText) {
    ...
    public function deleteComment(Entity $entity, Comment $comment)
    {
        $comment->delete();
        ...
        //redirection with a message
        return back()->withSystemSuccess(__('Comment has been deleted!'));
    }
    ...
    public function deletePhoto(Entity $entity)
    {
        ...
        //redirection with a message
        return redirect()->route($this->namespace . 'list')->withSystemSuccess(__('Entity photo deleted!'));
    }
